...
seed()
  .then(async () => {
    ...
    formatLog({ name: '🟣 Files', value: await prisma.file.count() })
    ...
    formatLog({ name: '🟣 Invitations', value: await prisma.invitation.count() })
    formatLog({ name: '🟣 Jobs', value: await prisma.job.count() })
    formatLog({ name: '🟣 Leaders', value: await prisma.leader.count() })
    ...
    formatLog({ name: '🟣 Progresses', value: await prisma.progress.count() })
