...
export default function SelectMultipleMenu({ options, menuOnTop, addOption, disabled }: Props) {
  ...
  return (
    <div className="relative mb-3 w-full max-w-sm" ref={menuRef}>
      ...
      {!disabled && (
        <ul
          className={clsx(
            'menu absolute z-10 max-h-72 w-full flex-nowrap overflow-y-auto rounded-md border border-neutral-300 bg-white text-sm scrollbar-thin scrollbar-thumb-neutral-300/75', hidden && 'hidden', menuOnTop && 'bottom-full'
          )}
        >
          {filteredOptions.length === 0
            ...
            : filteredOptions.map(({ value, label }) => (
              <li
                tabIndex={-1} className="cursor-pointer rounded-md p-2 transition-colors hover:bg-neutral-200" key={value} onClick={() => { handleOptionClick(value) }}
              >
                {label}
              </li>
            ))}
