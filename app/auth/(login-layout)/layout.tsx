...
export default function LoginLayout({
  ...
  return (
    <>
      <img src={uri('/coso4.webp')} alt="Imagen decorativa en esquinas" className="pointer-events-none absolute right-0 top-0 hidden h-full w-2/4 rotate-180 md:block" />
      <img src={uri('/coso3.webp')} alt="Imagen decorativa en esquinas" className="pointer-events-none absolute left-0 top-0 block h-2/5 w-full md:hidden" />
      <div className="z-10 grid h-screen grid-cols-7 place-items-center">
        ...
        <section className="col-span-7 flex flex-col justify-center rounded-b-2xl rounded-t-none bg-white/10 p-6 shadow-lg backdrop-blur-sm md:order-2 md:col-span-2 md:w-5/6 md:rounded-2xl lg:w-9/12">
          <div className="flex flex-row items-center gap-2 md:flex-col md:items-end">
            ...
            <img src={uri('/undraw_appreciation.webp')} alt="Imagen de trabajo remoto" className="z-50 order-1 grid h-24 w-36 place-items-end md:order-2 md:h-auto md:w-48" />
          </div>
