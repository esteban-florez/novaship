...
export default async function SubtaskComponent({
  ...
  permissions.delete = subtask.task.personId === userId
  permissions.comment = subtask.task.personId === userId
  permissions.edit = subtask.task.personId === userId
          ...
          <Collapse title="Revisiones">
            {subtask.revisions.map((r) => {
              return (
                ...
                >
                  <p className="hover:text-primary text-neutral-600 inline-flex items-center">
                    {r.content}
